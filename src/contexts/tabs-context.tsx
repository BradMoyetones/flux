...
import { HOME_PATH, getRoute, routes } from '@/router/tab-routes'
...
interface TabsState {
  ...
  /** Open a route in a tab. See {@link OpenTabOptions} for de-duplication. */
  openTab: (path: string, options?: OpenTabOptions) => void
  /** Focus an already-open tab by id. */
  activateTab: (id: string) => void
  /** Close a tab by id (no-op for non-closable routes). */
...
function sanitize(tabs: OpenTab[], activeId: string) {
  ...
  const active = valid.some((tab) => tab.id === activeId)
    ? activeId
    : valid[0].id
    ...
    (set, get) => ({
      ...
      closeTab: (id) => {
        ...
        if (activeId === id) {
          ...
          nextActive = (next[index - 1] ?? next[0]).id
        }
...
export function useActivePath(): string {
  return useTabsStore(
    (state) =>
      state.tabs.find((tab) => tab.id === state.activeId)?.path ?? HOME_PATH,
  )
}
