...
export function FlowNode({
    ...
    return (
        ...
        >
            {/* Colored header */}
            <div className={cn('flex items-center gap-2.5 rounded-t-xl px-3.5 py-2.5', colors.header)}>
                <span className="grid size-7 place-items-center rounded-md bg-white/20">
                    <Icon className="size-4" />
                </span>
                <div className="min-w-0">
                    <p className="text-[11px] font-medium uppercase tracking-wide opacity-80">{kind}</p>
                ...
                <div className="space-y-1.5">
                    {fields.map((field) => (
                        ...
                        >
                            ...
                            <span className="truncate font-mono text-[11px] text-foreground">{field.value}</span>
                        </div>
                    ))}
                </div>
            </div>
